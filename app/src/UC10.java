...
class BookingService {
    ...
    synchronized void cancelReservation(String reservationId) {
        ...
        for (Booking b : bookingHistory) {
            if (b.reservationId.equals(reservationId)) {
                toCancel = b;
                break;
            }
        }
        ...
        if (toCancel == null) {
            System.out.println("Cancellation failed: Reservation does not exist or already cancelled - " + reservationId);
            return;
        }

        String roomId = toCancel.roomId;
        rollbackStack.remove(roomId);
        allocatedRooms.get(toCancel.roomType).remove(roomId);
        allRoomIds.remove(roomId);
        confirmedReservations.remove(reservationId);
        inventoryService.increment(toCancel.roomType);
        bookingHistory.remove(toCancel);

        System.out.println("Cancelled: " + reservationId + " -> Room " + roomId);
    }
}
...
public class UC10 {
    public static void main(String[] args) {
        ...
        bookingService.cancelReservation("REQ2");
        bookingService.cancelReservation("REQ4"); // invalid cancellation
        bookingService.cancelReservation("REQ1");
